...
The installer creates a private virtual environment at `~/.local/share/noaa-navionics/venv`, symlinks commands into `~/.local/bin`, installs the chartplotter autostart entry, uses noninteractive apt calls for unattended SSH deployment, ensures LightDM and X11 display-power tools are installed for graphical startup, ensures `vcgencmd` is available for Raspberry Pi power checks, and configures LightDM graphical autologin for the installing user. It tries `raspi-utils` first and falls back to `libraspberrypi-bin` for older Raspberry Pi OS images. It syncs the installed command symlinks, launchers, source revision file, desktop autostart entry, and user systemd unit files to disk. The Python code uses only the standard library. `opencpn` renders NOAA ENCs, `gpsd` shares one GPS feed between OpenCPN and this tool, and `chrony` can discipline the Pi clock from GPSD when network time is unavailable. The chart refresh timer and track logger are enabled for future boots during install, but provisioning starts them only after the onboard config and GPSD have been configured. Use `--skip-autologin` only for deliberate headless or development deployments.
