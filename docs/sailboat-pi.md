...
The installed chart refresh service runs `sync-charts` with retries, a two-hour systemd start timeout, delayed service-level retry attempts, `NoNewPrivileges`, a private temporary directory, `ProtectSystem=full`, `LockPersonality`, `RestrictSUIDSGID`, a private `0077` umask, and a 30-minute randomized timer delay so missed weekly refreshes do not always run immediately at boot beside chartplotter startup.
