...
The script requires an absolute `/dev/...` path without whitespace or quotes, validates the onboard app config and config directory before touching GPSD, backs up and syncs `/etc/default/gpsd`, replaces the GPSD config through a synced temporary file, reloads systemd, enables and restarts both `gpsd.socket` and `gpsd.service`, and updates `~/.config/noaa-navionics/config.ini` through a synced private `0600` atomic replacement.
