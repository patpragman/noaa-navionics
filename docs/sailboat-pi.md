...
Readiness rejects missing fix-quality, missing coordinates, non-finite, out-of-range, malformed numeric/hemisphere NMEA, and invalid `0,0` coordinates. GPSD parsing rejects non-finite positions and ignores non-finite optional speed, course, altitude, or HDOP values. When the receiver reports satellite count or HDOP, readiness also requires at least four satellites and HDOP no higher than 5. GPSD readiness merges recent SKY satellite/HDOP reports with TPV position fixes before applying that gate, and it still exits inside the configured GPS wait if GPSD only streams non-fix status messages.
