...
The anchor watch reads quality-checked GPSD or serial fixes from the onboard config, uses the first accepted fix as the anchor unless `--anchor-lat` and `--anchor-lon` are provided, prints distance updates, and exits non-zero with an audible terminal bell when drift exceeds `[anchor].radius_meters`. Use `--radius-meters N` for a one-off override, and `--interval-seconds N` to reduce normal update noise while still printing alarms immediately. Status reports and Pi verification include the configured anchor radius. It does not change charts, OpenCPN config, or services.
