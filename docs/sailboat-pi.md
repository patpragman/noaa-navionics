...
Use `--skip-gpsd` only when GPSD and the onboard config are already commissioned. If `--device` is supplied, the existing onboard config must name the same receiver. If unattended services or desktop autostart are still enabled, provisioning rejects missing config, placeholder GPS devices, mismatched requested devices, non-local GPSD hosts, volatile device names, nonexistent GPS paths, and disabled or inactive `gpsd.service` before it enables startup behavior. Skipping GPSD setup still leaves chrony GPS time setup enabled unless `--skip-gps-time` is also passed.
