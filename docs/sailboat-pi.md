...
Use `--skip-sync` only when the configured chart directory already has a fresh, complete NOAA chart manifest on trusted writable chart storage. If unattended services or desktop autostart are still enabled, provisioning rejects malformed or unsafe chart config plus missing, incomplete, symlinked chart-directory ancestors, misowned, or group/world-writable chart storage before it enables startup behavior.
