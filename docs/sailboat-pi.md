...
`noaa-navionics configure-opencpn`, below, configures OpenCPN to use the GPSD network source from the onboard config. The readiness path rejects extra enabled OpenCPN GPSD endpoints so a stale GPSD source cannot remain active beside the commissioned local receiver.
