...
READY reports also require structured Clock and Time Sync evidence, including a fresh timezone-aware clock timestamp and Raspberry Pi `SystemClockSynchronized=yes` unless the report explicitly records a non-Pi diagnostic skip. READY reports also require structured Pi Power and Pi Thermal evidence, including Raspberry Pi throttle flags and a finite temperature reading so under-voltage, throttling, and over-temperature conditions cannot be hidden in prose rows. READY reports also require structured Disk and Track Disk storage evidence, including absolute inspected paths, owner and mode, mount status, writability, and measured free space. Serial-mode READY reports also require structured GPS Device evidence proving the configured path is stable, non-volatile, present, and a character device, with `/dev/serial/by-id/*` paths verified as udev symlinks. GPSD-mode READY reports also require structured GPSD Config evidence proving `/etc/default/gpsd` is root-owned, non-writable, points only at the configured GPS device, disables USB auto-discovery, and enables `-n` immediate polling. READY reports also require structured OpenCPN and Display Power command evidence, including absolute root-owned non-symlinked executable paths in trusted system directories with non-writable command and directory modes.
