...
`gps-monitor --once` exits successfully only after a fresh timestamped GPS fix with satellite or HDOP quality data, so a position-only fix does not make dock diagnostics look healthy.
