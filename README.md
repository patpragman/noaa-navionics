...
The Raspberry Pi installer installs rsync, OpenCPN, GPSD, chrony, LightDM, X11 display-power utilities, and local Python build tooling on the Pi with noninteractive apt calls, replaces its guarded private virtual environment before installing this repo without build isolation or PEP 517 so it does not unexpectedly fetch Python build dependencies from PyPI, ensures Raspberry Pi power diagnostic utilities are available for `vcgencmd`, and only adds the Bookworm backports apt source when the Pi OS codename is Bookworm. Provisioning configures graphical autologin only after GPSD, charts, and the onboard config are commissioned.
