...
The installer creates or tightens app-owned user directories such as `~/.local/bin`, `~/.local/share/noaa-navionics`, `~/.config/noaa-navionics`, and `~/.config/systemd/user` to private `0700` permissions before installing files. It syncs installed command symlinks, launchers, source revision, and user systemd unit files to disk, and replaces command symlinks, helper launchers, and user systemd unit files through synced temporary files. Provisioning does the same directory hardening for launcher environment, user systemd, and desktop autostart paths before replacing refreshed user systemd unit files and the desktop autostart entry through synced temporary files and enabling or reloading startup behavior. Status reports parse user systemd unit install targets only after a no-follow descriptor read confirms the opened unit file is still regular, user-owned, and not group/world-writable. Pi verification rejects those startup and runtime config files or their key parent directories if they are symlinks, owned by the wrong account, or group/world-writable. The GPSD setup script validates the final NOAA Navionics app config, config directory, and `/etc/default/gpsd` target path before writing, rejects symlinked GPSD config path components, creates a synced root-owned private `0600` backup of `/etc/default/gpsd`, replaces it through a synced temporary file, reloads systemd, enables and restarts both `gpsd.socket` and `gpsd.service`, then updates the onboard `config.ini` through a synced private `0600` atomic replacement. Readiness reads GPSD and chrony config files only after a no-follow descriptor confirms the opened file is still regular, owned by the expected account, and not group/world-writable. Readiness also rejects symlinked, non-regular, writable, or misowned GPSD config files or path components before trusting the configured startup device. `scripts/configure_gps_time.sh` writes only the production chrony config path `/etc/chrony/chrony.conf` unless it is in dry-run mode, rejects symlinked, non-regular, writable, or misowned chrony config paths, refuses damaged managed block markers, creates a synced root-owned private `0600` backup of that file, replaces it through a synced temporary file, adds a managed GPSD `SHM 0` refclock block, restarts chrony, then restarts the GPSD socket and service so the daemons reconnect in the right order. Readiness also rejects unsafe chrony config paths or a missing/commented GPSD SHM refclock before trusting GPS-disciplined time. Desktop autologin rejects symlinked LightDM autologin path components or writable LightDM autologin paths, then writes the LightDM drop-in through the same synced temporary-file replacement before enabling graphical boot.
...
Launcher startup sets a private `0077` umask, rejects symlinked cache path components before creating `~/.cache/noaa-navionics`, tightens the user-owned `~/.cache` parent to `0700` before creating runtime files, rejects misowned cache directories, rejects symlinked or non-regular launcher logs, and appends output to the private `0600` file `~/.cache/noaa-navionics/chartplotter.log`, including OpenCPN's exit status if the chartplotter process stops.
...
It reads `NOAA_NAVIONICS_GPS_SECONDS`, optional `NOAA_NAVIONICS_WARNING_SECONDS`, optional `NOAA_NAVIONICS_READINESS_ATTEMPTS`, optional `NOAA_NAVIONICS_READINESS_RETRY_DELAY`, optional `NOAA_NAVIONICS_OPENCPN_RESTARTS`, optional `NOAA_NAVIONICS_OPENCPN_RESTART_DELAY`, and optional `NOAA_NAVIONICS_START_ON_FAILED_READINESS` from `~/.config/noaa-navionics/launcher.env` only after rejecting a missing launcher environment, symlinked launcher environment files or path components, misowned launcher environment files, launcher environment files that are not private `0600`, malformed launcher environment lines, and unknown launcher environment keys; ambient `NOAA_NAVIONICS_*` process environment variables are ignored so unattended boots use the commissioned private file. Missing or invalid launcher timing and fail-open values stop launcher startup instead of falling back to defaults. Status reports parse launcher settings only after a no-follow descriptor read confirms the file is still regular, user-owned, and private `0600`. The readiness report fails if the persisted launcher environment is missing, not regular, owned by the wrong account, group/world-writable, malformed, or contains unknown keys, so typos and unsafe startup policy files do not silently pass dock verification.
...
The launcher also keeps a synced cache-directory lock for the supervised OpenCPN session so overlapping desktop startup attempts cannot race each other while OpenCPN is still starting or running; it rejects symlinked lock paths before reading or cleaning them, leaves a lock untouched if it is swapped for a symlink before release cleanup, and if an old lock points at an unrelated reused PID, it refuses symlinked, misowned, or group/world-writable stale lock debris before removing the stale lock and continuing startup.
Before launching OpenCPN, the launcher resolves the command to an absolute executable path, rejects symlinked, non-regular, non-executable, group/world-writable, or unexpectedly owned binaries and command directories, requires a root-owned OpenCPN executable and executable directory on Raspberry Pi hardware, and logs the resolved path used for startup. Pi verification independently requires the deployed OpenCPN command and command directory to be root-owned trusted paths.
...
The included chart sync service retries transient network failures, allows up to two hours for slow NOAA downloads, runs with `NoNewPrivileges`, a private temporary directory, `ProtectSystem=full`, and a private `0077` umask, and asks systemd for delayed retry attempts if the whole run still fails.
