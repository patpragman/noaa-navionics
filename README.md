...
For direct serial checks, `preflight --gps-device` accepts `--gps-baud`; `status-report` uses the baud from `~/.config/noaa-navionics/config.ini`. A sample-based `status-report --gps-sample` substitutes the live fix read but still checks OpenCPN's GPSD connection when the onboard config is in `gpsd` mode. Manual `gps-monitor --device` and `log-track --device` direct serial runs also reject `/dev/serial/by-id/...` paths that are broken, exist but are not actual udev symlinks, or do not resolve to a character device. Direct serial readiness rejects stale, future-dated, and untimestamped NMEA fixes too.
