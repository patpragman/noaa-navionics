...
GPS readiness rejects missing fix-quality, missing coordinates, non-finite, out-of-range, malformed numeric/hemisphere NMEA, and invalid `0,0` coordinates. GPSD parsing rejects non-finite positions and ignores non-finite optional speed, course, altitude, or HDOP values. When a receiver reports quality fields, it also rejects weak fixes with fewer than four satellites or HDOP above 5. GPSD readiness merges recent SKY satellite/HDOP reports with TPV position fixes before applying that gate, and it still exits inside the wait window if GPSD only streams non-fix status messages.
