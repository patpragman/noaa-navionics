...
Preflight and Pi verification require a trusted root-owned `xset` from `x11-xserver-utils` so this display-awake step is available. Strict chartplotter verification also requires `pgrep` from `procps` so duplicate and live-process checks do not depend on a minimal base image.
