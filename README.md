...
Before launching OpenCPN, the launcher resolves the command to an absolute executable path, rejects symlinked, non-regular, non-executable, group/world-writable, or unexpectedly owned binaries, and logs the resolved path used for startup. Pi verification additionally requires the deployed OpenCPN command to be a root-owned trusted executable.
