...
For onboard `gpsd` mode, `gpsd_host` must be local (`127.0.0.1`, `localhost`, or `::1`) and `gps.device` must name the attached receiver, so OpenCPN, readiness checks, and the installed GPSD service all use the Pi's GPS peripheral.
