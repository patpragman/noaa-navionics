...
Provisioning persists the chosen GPS wait and OpenCPN restart policy in the private `0600` file `~/.config/noaa-navionics/launcher.env` through a synced temporary file and atomic replacement so the boot readiness service and desktop chartplotter launcher use the same cold-start window and crash-restart behavior after reboot. The boot readiness service does not use systemd `EnvironmentFile`; it passes that launcher environment path to `status-report`, which reads the GPS wait through the same no-follow, private-file validation used by status reports.
...
The installer creates or tightens app-owned user directories such as `~/.local/bin`, `~/.local/share/noaa-navionics`, `~/.config/noaa-navionics`, and `~/.config/systemd/user` to private `0700` permissions before installing files. It syncs installed command symlinks, launchers, source revision, and user systemd unit files to disk, and replaces command symlinks, helper launchers, and user systemd unit files through synced temporary files. Installer venv cleanup refuses Python runtimes without symlink-attack-resistant `shutil.rmtree` before removing the previous private virtual environment. Provisioning does the same directory hardening for launcher environment, user systemd, and desktop autostart paths before replacing refreshed user systemd unit files and the desktop autostart entry through synced temporary files and enabling or reloading startup behavior. Provisioning revalidates launcher environment and user-file targets immediately before promotion, then verifies the promoted launcher environment and promoted user service/autostart files through no-follow descriptors before enabling startup services. Status reports and Pi verification parse user systemd unit install targets only after a no-follow descriptor read confirms the opened unit file is still regular, user-owned, and not group/world-writable. Pi verification rejects those startup and runtime config files or their key parent directories if they are symlinks, owned by the wrong account, or group/world-writable. The GPSD setup script reads any existing onboard app config through a no-follow descriptor, validates the final NOAA Navionics app config, config directory, and `/etc/default/gpsd` target path before writing, rejects symlinked GPSD config path components, creates a synced root-owned private `0600` backup of `/etc/default/gpsd`, replaces it through a synced temporary file, reloads systemd, enables and restarts both `gpsd.socket` and `gpsd.service`, then updates the onboard `config.ini` through a synced private `0600` atomic replacement. GPS time setup reads existing chrony config, and readiness and production skip checks read GPSD and chrony config files, only after a no-follow descriptor confirms the opened file is still regular, owned by the expected account, and not group/world-writable. Readiness also rejects symlinked, non-regular, writable, or misowned GPSD config files or path components before trusting the configured startup device. `scripts/configure_gps_time.sh` writes only the production chrony config path `/etc/chrony/chrony.conf` unless it is in dry-run mode, rejects symlinked, non-regular, writable, or misowned chrony config paths, refuses damaged managed block markers, creates a synced root-owned private `0600` backup of that file, replaces it through a synced temporary file, adds a managed GPSD `SHM 0` refclock block, restarts chrony, then restarts the GPSD socket and service so the daemons reconnect in the right order. Readiness also rejects unsafe chrony config paths or a missing/commented GPSD SHM refclock before trusting GPS-disciplined time. Desktop autologin rejects symlinked LightDM autologin path components or writable LightDM autologin paths, then writes the LightDM drop-in through the same synced temporary-file replacement before enabling graphical boot. GPSD setup, GPS time setup, and desktop autologin sync promoted root files only after reopening them as regular no-follow file descriptors.
...
Launcher startup sets a private `0077` umask, rejects symlinked cache path components before creating `~/.cache/noaa-navionics`, tightens the user-owned `~/.cache` parent to `0700` before creating runtime files, rejects misowned cache directories, rejects symlinked, non-regular, or misowned launcher logs, and appends output to the private `0600` file `~/.cache/noaa-navionics/chartplotter.log` only after opening it through a no-follow descriptor, including OpenCPN's exit status if the chartplotter process stops.
...
The launcher also keeps a synced cache-directory lock for the supervised OpenCPN session so overlapping desktop startup attempts cannot race each other while OpenCPN is still starting or running; it rejects symlinked lock paths before reading or cleaning them, checks NUL-delimited process arguments rather than substring-matching lock owners, leaves a lock untouched if it is swapped for a symlink before release cleanup, and if an old lock points at an unrelated reused PID, it refuses symlinked, misowned, or group/world-writable stale lock debris before removing the stale lock and continuing startup.
...
For direct serial checks, `preflight --gps-device` accepts `--gps-baud`; `status-report` uses the baud from `~/.config/noaa-navionics/config.ini`. Manual `gps-monitor --device` and `log-track --device` direct serial runs also reject `/dev/serial/by-id/...` paths that exist but are not actual udev symlinks. Direct serial readiness rejects stale, future-dated, and untimestamped NMEA fixes too.
