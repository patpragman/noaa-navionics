...
Preflight and Pi verification require a trusted root-owned `xset` from `x11-xserver-utils` so this display-awake step is available. Strict chartplotter verification also requires trusted root-owned `pgrep`, `vcgencmd`, `chronyc`, `gpsd`, and `cgps` commands so duplicate-process, power, time, and GPS checks do not depend on untrusted command paths or a minimal base image.
