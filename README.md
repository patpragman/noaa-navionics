...
Use a `/dev/serial/by-id/` path when possible; config validation, GPSD setup, and verification fail volatile USB names such as `/dev/ttyUSB0` or `/dev/ttyACM0`, and reject unrecognized device paths that are not one of the documented stable aliases.
