...
Use `--skip-gpsd` only when the onboard config already names a commissioned local GPSD receiver; production provisioning rejects missing, placeholder, remote, volatile, or nonexistent GPS config and disabled or inactive `gpsd.service` before enabling unattended startup. This does not skip chrony GPS time setup; add `--skip-gps-time` only when that is already commissioned too.
...
The dock test requests reboot with noninteractive `sudo -n reboot` and fails clearly if the SSH user cannot reboot without a password prompt. It also passes the requested GPS device into verification so the onboard config and GPSD daemon must still point at the intended receiver. After the reboot, the dock test requires the Pi boot ID to change, then uses the stricter verify mode that waits briefly for desktop autostart, requires the existing readiness status report and chartplotter launcher log to be fresh for the current boot, rejects launcher logs that started OpenCPN after failed readiness, and requires an `opencpn` process owned by the deployed user to remain running through a short stability check.
