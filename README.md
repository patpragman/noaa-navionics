...
The Raspberry Pi installer installs OpenCPN, GPSD, chrony, LightDM, and X11 display-power utilities on the Pi, ensures Raspberry Pi power diagnostic utilities are available for `vcgencmd`, configures graphical autologin for the installing user, and only adds the Bookworm backports apt source when the Pi OS codename is Bookworm.
