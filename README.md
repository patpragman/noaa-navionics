...
Use a `/dev/serial/by-id/` path when possible; verification checks that the configured GPSD device still exists and fails volatile USB names such as `/dev/ttyUSB0` or `/dev/ttyACM0`.
