...
grep -q 'ssh_batch_options=(-o BatchMode=yes)' scripts/deploy_to_pi.sh
grep -q 'ssh_connect_options=(-o BatchMode=yes -o ConnectTimeout=10)' scripts/deploy_to_pi.sh
grep -q 'rsync -az --delete -e "ssh -o BatchMode=yes"' scripts/deploy_to_pi.sh
...
grep -Fq 'ssh -T -o BatchMode=yes "$target"' scripts/verify_pi.sh
...
grep -q 'ssh -T -o BatchMode=yes' scripts/verify_pi.sh
