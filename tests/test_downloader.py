...
class GuiTests(unittest.TestCase):
    ...
    def test_status_gui_status_refresh_rings_when_ready_report_becomes_not_ready(self):
        class FakeVar:
            def __init__(self):
                self.value = None
...
class ManifestTests(unittest.TestCase):
    ...
    def test_forced_download_rejects_oversized_zip_member_before_replacing_archive(self):
        ...
        try:
            downloader_module.MAX_ZIP_MEMBER_UNCOMPRESSED_BYTES = 3
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                good_zip = root / "good.zip"
                with zipfile.ZipFile(good_zip, "w") as archive:
                    archive.writestr("US5AK3CM/US5AK3CM.000", "ok")
                output = root / "charts"
                package = Package("State AK", good_zip.as_uri(), "AK_ENCs.zip")
                download_package(package, output, extract=True, keep_zip=True, force=True)
                archive_path = output / "AK_ENCs.zip"
                original_archive_bytes = archive_path.read_bytes()
                original_manifest = read_manifest(output)
                oversized_zip = root / "oversized.zip"
                with zipfile.ZipFile(oversized_zip, "w") as archive:
                    archive.writestr("US5AK3CM/US5AK3CM.000", "cell")
                oversized_package = Package("State AK", oversized_zip.as_uri(), "AK_ENCs.zip")

                with self.assertRaisesRegex(RuntimeError, "downloaded ZIP member is too large"):
                    download_package(oversized_package, output, extract=True, keep_zip=True, force=True)

                self.assertEqual(archive_path.read_bytes(), original_archive_bytes)
                self.assertEqual(read_manifest(output), original_manifest)
                self.assertEqual((output / "AK_ENCs" / "US5AK3CM" / "US5AK3CM.000").read_text(encoding="ascii"), "ok")
                self.assertFalse((output / "AK_ENCs.zip.part").exists())
        finally:
            downloader_module.MAX_ZIP_MEMBER_UNCOMPRESSED_BYTES = original_limit
    ...
    def test_manifest_archive_rejects_retained_zip_with_oversized_member(self):
        ...
        try:
            health_module.MAX_ZIP_MEMBER_UNCOMPRESSED_BYTES = 3
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                archive = root / "AK_ENCs.zip"
                with zipfile.ZipFile(archive, "w") as zip_handle:
                ...
                self.assertFalse(result.ok)
                self.assertIn("retained chart archive member is too large", result.detail)
        finally:
    ...
    def test_extract_zip_rejects_too_many_members_before_staging(self):
        original_limit = downloader_module.MAX_ZIP_MEMBERS
        try:
            downloader_module.MAX_ZIP_MEMBERS = 1
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                archive = root / "charts.zip"
                with zipfile.ZipFile(archive, "w") as zip_file:
                    zip_file.writestr("US5AK3CM/US5AK3CM.000", "cell")
                    zip_file.writestr("US5AK3CM/US5AK3CM.001", "extra")
                destination = root / "AK_ENCs"

                with self.assertRaisesRegex(RuntimeError, "chart ZIP has too many members"):
                    extract_zip(archive, destination)
    ...
    def test_extract_zip_rejects_total_uncompressed_size_before_staging(self):
        ...
        try:
            downloader_module.MAX_ZIP_TOTAL_UNCOMPRESSED_BYTES = 5
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                archive = root / "charts.zip"
                with zipfile.ZipFile(archive, "w") as zip_file:
                    zip_file.writestr("US5AK3CM/US5AK3CM.000", "abc")
                    zip_file.writestr("US5AK3CM/US5AK3CM.001", "def")
                destination = root / "AK_ENCs"

                with self.assertRaisesRegex(RuntimeError, "chart ZIP uncompressed size is too large"):
                    extract_zip(archive, destination)

                self.assertFalse(destination.exists())
                self.assertFalse(list(root.glob(".AK_ENCs.*.extracting")))
        finally:
            downloader_module.MAX_ZIP_TOTAL_UNCOMPRESSED_BYTES = original_limit
...
class StatusReportTests(unittest.TestCase):
    ...
    def test_status_report_ready_requires_structured_chart_readiness_evidence(self):
        ...
        cases = [
            ...
            ("Charts", charts_data(has_unextracted_zips=True), "found unextracted ZIP chart artifacts"),
            ("Charts", charts_data(zip_samples=["/charts/AK_ENCs.zip"]), "ZIP sample list is not empty"),
            ...
            ("Charts", charts_data(enc_cell_samples=["/other/US5AK00M.000"]), "ENC cell sample path is outside chart output"),
    ...
    def test_status_report_ready_requires_valid_track_log_summary(self):
        ...
        cases = [
            ...
            ({"track_log": {**valid_track_log, "track_output": "charts"}}, "track_output is not absolute"),
            ({"track_log": {**valid_track_log, "tracks_dir": "tracks"}}, "tracks_dir is not absolute"),
            ({"track_log": {**valid_track_log, "tracks_dir": "/other/tracks"}}, "tracks_dir does not match track_output"),
            ...
            ({"track_log": {**valid_track_log, "latest_path": "track-20260701.gpx"}}, "latest_path is not absolute"),
            ({"track_log": {**valid_track_log, "latest_path": "/charts/other/track-20260701.gpx"}}, "latest_path is not under tracks_dir"),
            ({"track_log": {**valid_track_log, "latest_path": "/charts/tracks/latest.txt"}}, "latest_path is not a track-*.gpx file"),
            ({"track_log": {key: value for key, value in valid_track_log.items() if key != "tracks_mode"}}, "tracks_mode is missing or invalid"),
            ({"track_log": {**valid_track_log, "tracks_mode": "0777"}}, "tracks_mode 0777 is not private"),
            ({"track_log": {key: value for key, value in valid_track_log.items() if key != "latest_mode"}}, "latest_mode is missing or invalid"),
            ({"track_log": {**valid_track_log, "latest_mode": "0644"}}, "latest_mode 0644 is not private"),
    ...
    def test_status_snapshot_validators_require_structured_gps_and_track_summaries(self):
        expectations = {
            "scripts/pre_trip_prepare_pi.sh": (
                ...
                "pre-departure status snapshot JSON Charts found unextracted ZIP chart artifacts",
                "pre-departure status snapshot JSON Charts ZIP sample list is not empty",
                "pre-departure status snapshot JSON Charts ENC cell sample path is outside chart_output",
                "pre-departure status snapshot JSON OpenCPN Charts parsed directories do not include configured chart output",
                "pre-departure status snapshot JSON OpenCPN GPSD parsed connections do not include configured endpoint",
                "pre-departure status snapshot JSON GPSD Config devices do not match configured GPS device",
                "pre-departure status snapshot JSON Chrony Config refclock line is not the managed GPSD SHM source",
                "pre-departure status snapshot JSON GPS Time Source did not prove selected or combined GPS time",
                ...
                "pre-departure status snapshot JSON track_log latest_path is not under tracks_dir",
                "pre-departure status snapshot JSON track_log latest_path is not a track-*.gpx file",
                "pre-departure status snapshot JSON track_log {field} is missing or invalid",
                "pre-departure status snapshot JSON track_log {field} is not private",
            ...
            "scripts/post_trip_collect_pi.sh": (
                ...
                "status snapshot JSON Charts found unextracted ZIP chart artifacts",
                "status snapshot JSON Charts ZIP sample list is not empty",
                "status snapshot JSON Charts ENC cell sample path is outside chart_output",
                "status snapshot JSON OpenCPN Charts parsed directories do not include configured chart output",
                "status snapshot JSON OpenCPN GPSD parsed connections do not include configured endpoint",
                "status snapshot JSON GPSD Config devices do not match configured GPS device",
                "status snapshot JSON Chrony Config refclock line is not the managed GPSD SHM source",
                "status snapshot JSON GPS Time Source did not prove selected or combined GPS time",
                ...
                "status snapshot JSON track_log latest_path is not under tracks_dir",
                "status snapshot JSON track_log latest_path is not a track-*.gpx file",
                "status snapshot JSON track_log {field} is missing or invalid",
                "status snapshot JSON track_log {field} is not private",
    ...
    def test_status_snapshot_validators_require_status_desktop_launcher_evidence(self):
        expectations = {
            "scripts/pre_trip_prepare_pi.sh": "pre-departure status snapshot JSON",
            "scripts/verify_pi_recovery_exports.sh": "pre-departure status snapshot JSON",
            "scripts/post_trip_collect_pi.sh": "status snapshot JSON",
        ...
        for script, prefix in expectations.items():
            with self.subTest(script=script):
                ...
                for expected in (
                    ...
                    f"{prefix} missing desktop section",
                    f"{prefix} missing status GUI desktop launcher evidence",
                    f"{prefix} status GUI desktop launcher path is not absolute",
                    f"{prefix} status GUI desktop launcher path is a symlink or missing symlink status",
                    f"{prefix} status GUI desktop launcher missing path_symlink_component",
                    f"{prefix} status GUI desktop launcher path contains a symlink",
                    f"{prefix} {{label}} owner is invalid",
                    'snapshot_uid(status_launcher.get("uid"), label="status GUI desktop launcher"',
                    f"{prefix} status GUI desktop launcher is group/world writable",
                    f"{prefix} status GUI desktop launcher is not user executable",
                    f"{prefix} status GUI desktop launcher values were not parsed",
                    f"{prefix} status GUI desktop launcher {{key}} does not match expected value",
                    "noaa-navionics-status-gui",
                    f"{prefix} status GUI desktop launcher must not be configured for autostart",
                    'validate_snapshot_status_launcher(',
                ):
                    with self.subTest(expected=expected):
                        self.assertIn(expected, source)
